...
from .SAE_old import SAE  # this is for installing package
# except:
...
class DescModel(object):
    ...
    def pretrain(self, use_clus_centroids=None):
        ...
        if self.use_ae_weights:
            print("Checking whether %s  exists in the directory" % str(os.path.join(self.save_dir, 'ae.weights.h5')))
            if not os.path.isfile(os.path.join(self.save_dir, "ae.weights.h5")):
                print("The file ae.weights.h5 doesnt exist. Will rerun autoencoder")
                if self.is_stacked:
                ...
                sae.autoencoders.load_weights(os.path.join(self.save_dir, "ae.weights.h5"))
                self.autoencoder = sae.autoencoders
                self.encoder = sae.encoder
        else:
        ...
        if not os.path.isfile(os.path.join(self.save_dir, "ae.weights.h5")):
            self.autoencoder.save_weights(os.path.join(self.save_dir, 'ae.weights.h5'))
            self.encoder.save_weights(os.path.join(self.save_dir, 'encoder.weights.h5'))
            print('Pretrained weights are saved to %s /ae.weights.h5' % self.save_dir)
        ...
        if use_clus_centroids is not None:
            ...
            print(''.join(['...number of clusters is unknown, Initialize cluster centroid using ', self.method, ' method']))
            # can be replaced by other clustering methods
            # using leiden, louvain methods in scanpy
    ...
    def fit_on_batch(self, maxiter=1e4, update_interval=200, save_encoder_step=4):
        ...
        for ite in range(int(maxiter)):
            if self.save_encoder_weights and ite % (save_encoder_step*update_interval) == 0:
                self.encoder.save_weights(os.path.join(self.save_dir, 'encoder_weights_resolution_'+str(self.resolution)+"_"+str(ite)+'.weights.h5'))
                print('Fine tuning encoder weights are saved to %s/encoder.weights.h5' % self.save_dir)
        ...
        # save encoder model
        self.encoder.save(os.path.join(self.save_dir, "encoder_model.keras"))
        # load model
...
if __name__ == "__main__":
    ...
    desc = DescModel(dims=dims, x=x, clustering_resolution=0.8, use_ae_weights=True, epochs_fit=0.4)
    desc.model.summary()
    t0 = get_time()
